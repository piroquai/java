 ...
 */
public class C34Rename_by_Limited_Receiver implements ITest{
    ...
    public void performTest(Vector<String> buf) {
        Proc.setBuf(buf);
        Proc.setGLP(" C34 Rename Shared folder by limited"); //Place global log prefix
        Proc.setLogScenarioPrefix("C34");
        log(" Rename Shared folder C.3.4"); //Place test description
        try {
            ...
            if (RunRename.custom) {
                ...
                EditorProc.changeAccount(UserProc.getUserCustom(5));
            } else {
                Proc.setLogStepPrefix("IIb");
                EditorProc.changeAccount(UserProc.getUser(3));
            }
            sleepy(2);
