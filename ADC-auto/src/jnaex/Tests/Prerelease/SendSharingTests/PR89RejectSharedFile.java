 ...
 */
public class PR89RejectSharedFile implements ITest{
    ...
    public void performTest(Vector<String> buf) {
        Proc.setBuf(buf);
        Proc.setGLP(" PR89 Reject shared file"); //Place global log prefix
        Proc.setLogScenarioPrefix("PR89");
        log("PR89 Reject shared file"); //Place test description
        try {
...
//            String R_password = SSTestBlock.passwRReceive;
            ...
            sleepy(2);
            String  newName = EditorProc.getNewName(SSTestBlock.nameShareFileR, testUser, "share");
            Proc.setLogStepPrefix("III");
            EditorProc.handleSync(newName, testUser, 1, true);
            sleepy(2);
            Proc.setLogStepPrefix("IV");
            EditorProc.searchFile(newName,false);
            sleepy(3);
