 ...
 */
public class PR8BAcceptFileWithSync implements ITest{
    ...
    public void performTest(Vector<String> buf) {
        Proc.setBuf(buf);
        Proc.setGLP(" PR8B Accept send file with sync"); //Place global log prefix
        Proc.setLogScenarioPrefix("PR8B");
        log("  PR8B Accept send file with sync"); //Place test description
        try {
...
//            String R_password = SSTestBlock.passwOReceive;
            ...
            sleepy(2);
            String  newName = EditorProc.getNewName(SSTestBlock.nameSendFile, testUser, "send");
