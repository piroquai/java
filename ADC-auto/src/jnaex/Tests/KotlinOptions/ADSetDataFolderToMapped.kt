 ...
 */
class ADSetDataFolderToMapped : KotlinITest {
    override fun performTest(test: KotlinTest){
        Proc.setGLP("AccountData:SetDataFolderToMapped")
        Proc.setLogScenarioPrefix("ADSDFMapped")
        var succeed = false
        ...
        try {
            ...
            sleepy(1)
            setLogStepPrefix("IV")
            handleBrowseForFolderDialog(optADDataFolderMapped,DialogActions.OK)
            sleepy(8)
            ...
            setLogStepPrefix("VI")
            deleteEverythingAt(optADDataFolderMapped)
        } catch (e : Exception) {
