 ...
 */
class ADSetDataFolderToLocal : KotlinITest {
    override fun performTest(test: KotlinTest){
        Proc.setGLP("AccountData:SetDataFolderToLocal")
        Proc.setLogScenarioPrefix("ADSDFLocal")
        var succeed = false
        ...
        try {
            ...
            sleepy(1)
            setLogStepPrefix("IV")
            handleBrowseForFolderDialog(optADDataFolderLocal,DialogActions.OK)
            sleepy(8)
            ...
            setLogStepPrefix("VI")
            deleteEverythingAt(optADDataFolderLocal)
        } catch (e : Exception) {
